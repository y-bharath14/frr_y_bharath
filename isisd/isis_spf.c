				...
				if (flex_algo_id_valid(spftree->algorithm) &&
				    (!sr_algorithm_participated(
				     ...
				     isis_flex_algo_constraint_drop(spftree,
								    lsp, er) ||
				     !isis_flex_algo_extended_is_metric(
					     spftree, er->metric, er->subtlvs,
					     &metric)))
					continue;
#endif /* ifndef FABRICD */
...
#ifndef FABRICD
			if (flex_algo_id_valid(spftree->algorithm) &&
			    isis_is_prefix_attr_redist_ext(
				    r->subtlvs, spftree->algorithm)) {
				bool rc = isis_flex_algo_prefix_metric(
					r->subtlvs, r->metric, spftree->area,
					spftree->algorithm, &metric);
				if (rc == false) {
					if (IS_DEBUG_SPF_EVENTS)
						zlog_debug(
							"%s: can't get metric for prefix %pFX",
							__func__, &r->prefix);
					continue;
				}
			} else
				  ...
				  struct isis_vertex *parent)
{
	...
	for (ALL_LIST_ELEMENTS_RO(spftree->sadj_list, node, sadj)) {
		...
		if (CHECK_FLAG(spftree->flags, F_SPFTREE_HOPCOUNT_METRIC))
			 ...
			 * variable */
			if (!isis_flex_algo_extended_is_metric(
				    spftree, sadj->metric, sadj->subtlvs,
				    &metric)) {
				if (IS_DEBUG_SPF_EVENTS)
