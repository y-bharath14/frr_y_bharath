...
def test_p2mp_broadcast_neighbor_filter():
    ...
    neighbor_filter_cfg = (
        tgen.net["r1"]
        .cmd('vtysh -c "show running ospfd" | grep "^ ip ospf neighbor-filter nbr-filter"')
        .rstrip()
    )
    assertmsg = "'ip ospf neighbor-filter nbr-filter' applied, but not present in configuration"
    ...
    step("Verify neighbor-filter is now applied to r1 interface and neighbors still adjacent");
    ...
    step("Verify neighbor-filter is now applied to r1 interface and r4 is no longer adjacent");
    ...
    step("Verify neighbor-filter is now applied to r1 interface and r2 is no longer adjacent");
    ...
    assertmsg = (
        "'ip ospf neighbor' not applied, but present in R1 configuration"
    )
    assert rc, assertmsg
