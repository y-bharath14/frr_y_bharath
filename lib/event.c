...
struct event_loop *event_master_create(const char *name)
{
	...
	if (-1 ==
	    epoll_ctl(rv->handler.epoll_fd, EPOLL_CTL_ADD, rv->io_pipe[0],
		      &pipe_read_ev)) {
		...
		if (hash_ev) {
			...
			if (S_ISREG(fd_stat.st_mode)) {
				...
				if (i < m->handler.regular_revent_count) {
				   ...
				   epoll_ctl(m->handler.epoll_fd, EPOLL_CTL_MOD,
					     fd, &set_ev)) {
	...
	if (set_ev.events == 0) {
		...
		if (S_ISREG(fd_stat.st_mode)) {
			...
			master->handler
			   ...
			   -1 ==
				   epoll_ctl(master->handler.epoll_fd,
		...
		if (S_ISREG(fd_stat.st_mode)) {
			...
			if (i < master->handler.regular_revent_count) {
			   ...
			   epoll_ctl(master->handler.epoll_fd, EPOLL_CTL_MOD,
				     fd, &set_ev)) {
	...
	if (fd_closed || (revents[*i].events & (EPOLLHUP | EPOLLERR))) {
		if (S_ISREG(fd_stat.st_mode)) {
			...
			*i = *i - 1;
			   ...
			   -1 ==
				   epoll_ctl(m->handler.epoll_fd, EPOLL_CTL_DEL,
					     fd, NULL)) {
			/* Not regular file, remove the fd from the epoll set */
			zlog_debug("%s: EPOLL_CTL_DEL error, errno: %d",
				   __func__, errno);
